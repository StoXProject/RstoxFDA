...
## Example
As an example, we will demonstrate estimation with data from the catch lottery for North-sea Herring in 2022. This is an unequal probability sampling program, which selects hauls in real-time from an active fishery, with selection probabilities proportional to the first reports of catch size. 
...
```{r}
warning("Need something about domains, PSU domains and straificationVariables.")
```


We can then estimate abundance, frequencies, totals and means for each PSU. We need to specify the domain we want to estimate for (we provide age so that we get estimates for each age group). We also need to specify which variables we want means and totals for (we specify weight and length):
...
For domains / age grups with 0 abundance, totals are 0, and means are NaN.
...
The function _RstoxFDA::AnalyticalRatioEstimate_ also supports other kinds of ratio estimation based on only frequencies and mean weights, so that a total abundance estimate can be obtained even for sampling programs with less detailed information about the samling parameters.

## StoX project

In summary a StoX project implementing this kind of analysis, would extend upon the data-preparation pipeline ([StoX FDA data preparation (baseline)](Stox-data-preparation.html)) with the following kind of processes:
...
|  | DefineIndividualSamplingParameters | Defines sampling parameters for each individual, within PSU |
|  | AssignPSUSamplingParameters |  Matches selections to data records, handles non-response corrections |
|  | AnalyticalPSUEstimate |  Estimates abundance, frequencies, totals and means for each PSU |
|  | AnalyticalPopulationEstimate | Estimates abundance, frequencies, totals and means for the population of interest |
|  | AnalyticalRatioEstimate |  Improves estimates with census information. Provides Abundance and totals when only frequencies and means have been estimated in earlier steps. |
