...
Within the IMR-computer-network both landing data and sample data can be obtained from the [dataset explorer](http://datasetexplorer.hi.no). Consider using the tools-menu to download landing data, and to compile sample data, if the pre-compiled data sets under 'commercial sampling' are not suitable. Otherwise landing data is available in various formats as described at the [IMR intranet: Fiskeridata](https://hinnsiden.no/tema/Fiskeridata/Sider/side.aspx), and some data is publicly available directly from [FDIR](https://www.fiskeridir.no/Tall-og-analyse/AApne-data/AApne-datasett) (also in Norwegian).
...
# Analysis
The functions introduced in this vignette should enable you to use IMR samples and FDIR landings to obtain **RstoxData::StoxBiotic** and **RstoxData::StoxLanding** data with complete records and hamronized code, ready for further analysis or reporting. Relevant analysis and reporting functions will be introduced in other vignettes. See for instance [Stox Fisheries overview (report)](Stox-FDA-overview.html) for obtaining reports on fishing activity and how it is sampled.