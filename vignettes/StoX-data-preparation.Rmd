---
title: "StoX-data-preparation"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{StoX-data-preparation}
...
# FDA data preparation in StoX
This vignette introduces common tasks in data preparation for fisheries dependent analysis (FDA), and the functions that RstoxFDA provides for the StoX framework to support these tasks. These functions are mostly motivated by needs in fisheries-dependent analysis at the Institute of Marine Research (IMR), and supports common data-transformation, annotation and re-coding / code translation. FDA at IMR typically involve using census data from the Norwegian Directorate of Fisheries (FDIR) and reporting to the International Counsel for Exploration of the Seas (ICES). The data-preparation functions will be introduced in the context of setting up the StoX baseline for catch at age estimation with the package 'Reca', but most tasks are relevant for other estimation methods as well.
...
While this vignette introduces some tools to assist correct usage of FDA-data, no attempt is made at introducing unfamiliar users to the peculiarities of fisheries dependent sampling. For IMR samples a provisional guide to FDA-data can be found at: https://hinnsiden.no/organisasjon/fiskeridynamikk/PublishingImages/Sider/side/Kilder%20%e2%80%93%20fiskeriavhengige%20data.pdf.

## Sample data
StoX supports several input formats for biological samples, which can be read by the function _RstoxData::ReadBiotic_. These formats are not strictly distinguished in StoX and are all stored in the format *RstoxData::BioticData*. In order to standardize downstream functions, StoX also offers an internal format *RstoxData::StoxBioticData* and it is usually desirable to convert your biological data to that format via the RstoxData-function _RstoxData::StoxBiotic_. Some data transformation, filtering, and re-coding is necessary to do before converting to *RstoxData::StoxBioticData*, and some are necessary to do after converting. Data-preparation that can be done both before and after converting is generally desirable to do before, as error messages then provide a clearer link to the raw data.

### NMDbiotic
Biological samples at IMR are archived in a versioned format called NMDbiotic (https://www.imr.no/formats/nmdbiotic/). If the source for your biological data is NMDbiotic, you may find that information is recorded somewhat differently than what allows for straightforward conversion with _RstoxData::StoxBiotic_. For instance, you may find that fishing date is recorded as the end of the fishing operation, since that was the day the catch was sampled, and that the corresponding fields for the start of the fishing operation is missing. If not all data in the input file are relevant for analysis, the number of data issues to handle can be greatly reduced by first applying the necessary filtering with _RstoxData::FilterBiotic_

#### Fishing time information
You may also find that the time (hour and minute) is not filled, since that detail may not be available to samplers. Unfortunately, both of these fields ('stationstartdate' and 'stationstarttime' in *RstoxData::BioticData* / NMDbiotic) must be set in order for the conversion with _RstoxData::StoxBiotic_ to preserve time information. The downstream analysis may be as simple as annotating quarter or month to the data, and in that case it is clear that this kind of precision is not warranted. For situations like these, the function _RstoxFDA::SetStartDateBiotic_ allows you to impute the start date to be equal to the stop date of the fishing operation, and the function _RstoxFDA::SetTimeBiotic_ allows you to set an arbitrary time of the day to records where time is missing.

#### Position information
Some sampling still record position of catches only in terms of area codes. In order for visualisations and annotation of other area codes to be supported by *RstoxData::StoxBioticData*, positional data must be provided. If the area codes provided in the NMDbiotic records provide sufficient precision, you may consider imputing positions from area codes, using _RstoxFDA::SetAreaPositionsBiotic_. See the section 'Area codes' for more details. Before that can be done, some particularities of the IMR archive may need to be handled. Catch locations are often provided via the 'FDIR Statistical areas and locations', which is a two-code system where the first code represents an area (*RstoxData::BioticData*: area), and the second a finer area coding system within that area (*RstoxData::BioticData*: location). Both of these codes are defined as integers, and FDIR uses the convention to prefix single digit integers with a leading '0', E.g: '01' or '09'. In IMR records, several conventions have been used and the area '01' may be recorded as either '01', '1' or ' 1' (note the whitespace), and perhaps additional variants. This may be corrected by using _RstoxBase::DefineTranslation_ and _RstoxBase::TranslateBiotic_.

In addition to this FDIR revised their coding system in 2018, following a similar revision to the ICES areas. In NMDbiotic the old and revised are represented as to different systems of encoding (signified by the field NMDbiotic: system), but they are nearly identical and are in practice used interchangeably. _RstoxFDA::SetAreaPositionsBiotic_ may therefore have to occur twice in a StoX project. Once for system '2' (old convention) and once for system '10' (new convention).

#### Gear codes
NMDbiotic uses a four digit gear-coding system which is organized so that the two first digits code for gear-groups. If that coarser gear-definition is sufficient for analysis, downstream processing may be simplified by recoding the gear column to only the first two digits. This is supported with the function _RstoxFDA::SetShortGearBiotic_.

#### Non standard measurments
...
#### Quality filters
It has been common practice to remove extreme outliers from data. As most of these outliers are erroneous records, it is desirable to remove them from database records, or correct them if infromation is available. RstoxFDA provides some outlier-filters. These are mainly provided in order to reproduce legacy workflows, and may be relevant to apply mostly to older data. They may also be used in conjunction with report functions to verify that extreme outliers are uncommon or absent. The function _RstoxFDA::FilterAgeLengthOutliersStoxBiotic_ provides a filter against a von-Bertalannfy regression for removing extreme age-length relationships. The function _RstoxFDA::FilterWeightLengthOutliersStoxBiotic_ provides a filter against a log-linear model to remove extreme weight-length relationships.
...
## Census data
FDA analysis very often involve some kind of ratio-estimation dependent on a census of landed weight by species, or fishing effort by gear. At IMR we typically get such census data via mandatory reporting to FDIR. While RstoxData provides support for reading sales-notes from Fdir in a standardized format via the function _ReadLanding_, it is often convenient to use other formats. RstoxFDA therefore offers an alternative function for reading sales-notes: _ReadLandingFDA_ that allows for some additional formats, some of these only available internally at IMR. _ReadLandingFDA_ returns data that is compatible with _ReadLanding_ and can be used in any function that accepts data from _ReadLanding_. While RstoxData offers the function _FilterLanding_, that provides filtering options for that format, the StoX user interface performs best if data is converted to a more aggregated format before further processing. The RstoxData function _StoxLanding_ aggregates sales-notes and is supported by most further processing.
...
### StoX-project
Area codes can be used for imputing positions and annotating area codes to data with position, as examplified by the the sequence of processes in the table below:
...
## Extending StoxBioticData
Some information that may be stored in *RstoxData::BioticData* does not have corresponding columns in  *RstoxData::StoxBioticData*. These columns may be added by using _RstoxData::AddToStoxBiotic_, and may in some cases be used by downstream processes. For instance stock-typing or maturity may be needed, which is encoded in *RstoxData::BioticData* (NMDbiotic: maturationstage and NMDbiotic: otolithtype).

## Reca parameters
Some complex parameters for the Reca-models must be defined in the Stox-Baseline, if Reca is configured to use them. These are:
\describe{
\item{_RstoxFDA::DefineStockSplittingParameters_}{Defines uncertainty parameters for stock-splitting (coastal-cod analysis)}
\item{_RstoxFDA::DefineAgeErrorMatrix_}{Defines uncertainty parameters for age readings.}
\item{_RstoxFDA::DefineCarNeighbours_}{Define the neighbour-matrix for the CAR-variable.}
}
All of these require resource files, except _RstoxFDA::DefineCarNeighbours_, which may also be calculated from area defintion, as also described in the section 'Area codes'.