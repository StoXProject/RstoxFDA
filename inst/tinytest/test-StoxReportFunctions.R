...
rdiff <- (sum(catchAtAgeReportFlat$NbyAge$CatchAtAge) - sum(catchAtLengthAndAgeReportFlat$NbyLengthAge$CatchAtAge)) / sum(catchAtLengthAndAgeReportFlat$NbyLengthAge$CatchAtAge)
expect_true(abs(rdiff) < 1e-6)
catchAtLengthAndAgeReportFlatPlG <- RstoxFDA:::ReportRecaCatchAtLengthAndAge(catchAtAgeFlat, PlusGroup = 5, LengthInterval = 10)
rdiff <- (sum(catchAtAgeReportFlat$NbyAge$CatchAtAge) - sum(catchAtLengthAndAgeReportFlatPlG$NbyLengthAge$CatchAtAge)) / sum(catchAtLengthAndAgeReportFlatPlG$NbyLengthAge$CatchAtAge)
expect_true(abs(rdiff) < 1e-6)
