...
expect_equal(nrow(p), 87)
p <- LoadFdaStratumPolygon(NULL, "ICES.SubArea.2018")
expect_equal(nrow(p), length(unique(ICESareas$SubArea)))
p <- LoadFdaStratumPolygon(NULL, "ICES.Division.2018")
expect_equal(nrow(p), length(unique(paste(ICESareas$SubArea, ICESareas$Division, sep="."))))
p <- LoadFdaStratumPolygon(NULL, "ICES.SubDivision.2018")
expect_equal(nrow(p), length(unique(paste(ICESareas$SubArea[!is.na(ICESareas$SubDivision)], ICESareas$Division[!is.na(ICESareas$SubDivision)], ICESareas$SubDivision[!is.na(ICESareas$SubDivision)], sep="."))))
p <- LoadFdaStratumPolygon(NULL, "ICES.Unit.2018")
expect_equal(nrow(p), length(unique(paste(ICESareas$SubArea[!is.na(ICESareas$Unit)], ICESareas$Division[!is.na(ICESareas$Unit)], ICESareas$SubDivision[!is.na(ICESareas$Unit)], ICESareas$Unit[!is.na(ICESareas$Unit)], sep="."))))
...
temp <- DefinePeriod(NULL, TemporalCategory = "Quarter")
expect_true(data.table::is.data.table(temp))
expect_equal(nrow(temp), 4)
expect_equal(ncol(temp), 3)
...
classerror <- DefineStockSplittingParameters(FileName = classerorfile)

expect_true(is.StockSplittingParameters(classerror))
...
classNULL <- DefineStockSplittingParameters(NULL, FileName = classerorfile, UseProcessData = T)
...
expect_true(is.StockSplittingParameters(manual))
